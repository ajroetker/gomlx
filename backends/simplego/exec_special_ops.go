...
func readScalarInt(buf *Buffer) int {
	fnAny, err := readScalarIntDTypeMap.Get(buf.shape.DType)
	if err != nil {
		panic(err)
	}
	fn := fnAny.(func(flat any) int)
	return fn(buf.flat)
}
...
func execDynamicSlice(backend *Backend, node *Node, inputs []*Buffer, inputsOwned []bool) (*Buffer, error) {
	...
	fnAny, err := sliceDTypeMap.Get(node.shape.DType)
	if err != nil {
		return nil, err
	}
	fn := fnAny.(func(operand, output *Buffer, params *sliceNode))
...
func execDynamicUpdateSlice(backend *Backend, node *Node, inputs []*Buffer, inputsOwned []bool) (*Buffer, error) {
	...
	fnAny2, err2 := dynamicUpdateSliceDTypeMap.Get(node.shape.DType)
	if err2 != nil {
		return nil, err2
	}
	fn := fnAny2.(func(operand, update, output *Buffer, starts []int))
